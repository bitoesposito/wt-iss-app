...
export default function Widget (props: AllWidgetProps<IMConfig>) {
  ...
  const handle2dViewChange = useCallback((jmv: JimuMapView) => {
    ...
    jmv.whenJimuMapViewLoaded().then(() => {
      layer2dRef.current = getOrCreateLayer(jmv, LAYER_2D_ID, 'ISS Positions')
    })
  }, [getOrCreateLayer])
  ...
  const handle3dViewChange = useCallback((jmv: JimuMapView) => {
    if (!jmv) return
    jmv.whenJimuMapViewLoaded().then(() => {
      layer3dRef.current = getOrCreateLayer(jmv, LAYER_3D_ID, 'ISS 3D Model', 'absolute-height')
  ...
  const updateIssModel = useCallback((pos: IssPosition) => {
  ...
  const fetchPosition = useCallback(async () => {
    ...
    try {
      ...
      const pos: IssPosition = { latitude, longitude, altitude, timestamp: Date.now() }
  ...
  }, [config?.fetchUrl, config?.maxPositionCount, addPointTo2d, updateIssModel])
  ...
  useEffect(() => {
    ...
    void fetchPosition()
    const id = window.setInterval(() => { void fetchPosition() }, intervalMs)
    ...
    <div className='w-100 h-100 p-3'>
      ...
      {last && (
        <div className='mt-3'>
          ...
          <div>
            Lat: {last.latitude.toFixed(5)} | Lon: {last.longitude.toFixed(5)} | Alt: {last.altitude.toFixed(1)} km
          </div>
        </div>
      )}
